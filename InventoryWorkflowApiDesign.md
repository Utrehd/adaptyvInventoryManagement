...
| Keep the database model behind the API boundary | The frontend does not need to know whether availability comes from `InventoryLot`, `InventoryReservation`, or transaction history. |
...
```http
GET /v1/inventory-lots/expiring?before=2026-07-16&materialType=buffer&projectId=proj_123
```
...
```http
GET /v1/storage-locations/{locationId}/contents?includeNested=true&materialType=dna
```
...
```http
GET /v1/workflow-recipes?workflowType=binding&active=true
```
...
```http
GET /v1/workflow-runs/{workflowRunId}/material-plan?includeReruns=true
```
...
Uses the recipe, workflow run, planned rerun count, existing reservations, expiry, concentration, and location rules to answer whether the workflow can run.
...
```http
GET /v1/materials/{materialId}/inventory-lots?includeReservations=true
